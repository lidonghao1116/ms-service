...
public class GetWechatAuthrize {
	/**
	 * 微信授权链接URL
	 */
	...
	public static OAuthTokenBean getAccessToken(String code) {
		try {
			...
			requestUrl.append("?").append("appid=").append(Constants.APPID).append("&secret=").append(Constants.APPSECRET)
	...
	public static OAuthTokenBean refreshAccessToken(String refToken) {
		try {
			...
			requestUrl.append("?appid=").append(Constants.APPID).append("&grant_type=refresh_token&refresh_token=").append(refToken);
			JSONObject respToken = WechatUtils.httpRequest(requestUrl.toString(), "GET", null);
